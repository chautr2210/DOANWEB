...
if (!empty($_POST['category']) && !empty($_POST['color'])) {
    $selectedCategory = mysqli_real_escape_string($link, $_POST['category']);;
    $selectedColor = mysqli_real_escape_string($link, $_POST['color']);;
    $selectedColorId = $_POST['color'];
   
    if (!empty($selectedCategory) && !empty($selectedColor)) { // Kiểm tra xem giá trị tìm kiếm có rỗng hay không
       
    // Truy vấn để lấy id_category tương ứng với tên category
    $categoryQuery = "SELECT id_category FROM Category WHERE LOWER(name_category) = LOWER('$selectedCategory')";
    $categoryResult = chayTruyVanTraVeDL($link, $categoryQuery);

    if ($categoryResult->num_rows > 0) {
        $row = $categoryResult->fetch_assoc();
        $selectedCategoryId = $row['id_category'];}
    ...
    $colorResult = chayTruyVanTraVeDL($link, $colorQuery);
        ...
        if ($colorResult->num_rows > 0) {
            ...
            $query .= " WHERE p.id_category = $selectedCategoryId AND pd.id_color = $selectedColorId
...
<?php if (!empty($selectedCategory) && !empty($selectedColor)) {
echo '<h3 class="search-result">' . $resultCount . ' Results for "'. $selectedColor. ' '.$selectedCategory. '"</h3>';
}?>
</div>
