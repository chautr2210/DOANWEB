...
if (isset($_GET['category'])) {
    ...
    if (!empty($selectedCategory)) { // Kiểm tra xem giá trị tìm kiếm có rỗng hay không
        $selectedCategory = mysqli_real_escape_string($link, $selectedCategory); // Bảo mật giá trị tìm kiếm trước khi sử dụng    
...
$query = "SELECT p.*, pr.price, COUNT(pd.id_color) AS color_count
         ...
         WHERE p.id_category = $selectedCategoryId
...
echo '<h1 class="search-result" style="color: #000;  text-transform: uppercase;">' . $selectedCategory . '</h1>';
echo'</div>';}
